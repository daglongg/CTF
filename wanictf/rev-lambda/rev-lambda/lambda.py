...
def decode_step1(encoded_str):
    return ''.join(chr(int(c, 36) + 10) for c in encoded_str.split('_'))

def decode_step2(decoded_str1):
    return ''.join(chr(123 ^ ord(c)) for c in decoded_str1)

def decode_step3(decoded_str2):
    return ''.join(chr(ord(c) - 3) for c in decoded_str2)

def decode_step4(decoded_str3):
    return ''.join(chr(ord(c) + 12) for c in decoded_str3)

def find_flag(encoded_str):
    decoded_str1 = decode_step1(encoded_str)
    decoded_str2 = decode_step2(decoded_str1)
    decoded_str3 = decode_step3(decoded_str2)
    decoded_str4 = decode_step4(decoded_str3)
    return decoded_str4

encoded_str = '16_10_13_x_6t_4_1o_9_1j_7_9_1j_1o_3_6_c_1o_6r'

flag = find_flag(encoded_str)
